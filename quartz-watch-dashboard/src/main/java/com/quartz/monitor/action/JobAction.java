...
public class JobAction extends ActionSupport {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4844928711712936148L;
	...
	public String list() throws Exception {
		...
		if (schedulers != null && schedulers.size() > 0) {
			for (int i = 0; i < schedulers.size(); i++) {
				...
				List<Job> temp = instance.getJmxAdapter().getJobDetails(
						instance, scheduler);
				for (Job job : temp) {
	...
	public String add() throws Exception {
		...
		map.put("jobClass", JobContainer.getJobById(job.getJobClass())
				.getJobClass());
