...
@Slf4j
@Internal
public class RandomShardingStrategy implements ShardingStrategy {
    ...
    public List<Shard> allocateShards(int totalShards, String jobKey) {
        ...
        for (int i = 0; i < totalShards; i++) {
            NodeInfo node = activeNodes.get(random.nextInt(activeNodes.size()));
        ...
        log.info("Allocated {} shards to {} nodes using random strategy for job: {}", 
                 totalShards, shards.size(), jobKey);
